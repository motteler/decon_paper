...
% But the key idea is to fit for a separate linear correction for each
% channel using a large mostly cloudy dependent set, and then use the
% 49 profile fitting set---the same set we are using for most other
% tests here---as the independent set.  The separate functions
% guarantee no cross correlations are introduced, and the fitting
% profile test is more strict than for example splitting the cloudy
% set into dependent and independent sets, in the sense that the
% fitting profles are less correlated and the residuals for the
% fitting set are larger.
