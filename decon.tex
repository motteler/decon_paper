...
{\cris} by direct regression.  For that case let $\rtc$ be true
{\cris} and $\rta$ true {\airs} radiances from the dependent set,
find $X$ by regression to minimize $||X \rta - \rtc||_2$, and apply
$X$ to the independent set.  This can work well, at least in terms
of minimizing residuals, but introduces undesired explicit
correlations.  The correlations can be reduced with principal
component regression, but at the cost of larger residuals.
...
Gaussian response function of section \ref{decon} with $\fwhm = v /
R$, $dv = \fwhm / 2$, and with the $dv$-spaced channel steps
starting at $v_0$.  As with tests of the {\airs} to {\cris}
translation, true L1c is calculated by convolving kcarta radiances
with {\airs} L1c SRFs and true L1d by convolving with an L1d basis
at the desired resolving power.  L1c is translated to L1d by
deconvolution followed by reconvolution to the desired L1d basis,
and this is compared with true L1d.
