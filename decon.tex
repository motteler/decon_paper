...
we solve $r_a^t X^t = r_c^t$ for $X$ by regression.  This is in
contrast with the corrections of section \ref{airs2cris} and
\ref{airsL1d}.  There regression was used to find linear or
quadratic correction coefficients independently for each channel.
In contrast for direct regression each output channel can be a
linear combination of any number of input channels.
...
Unfortunately the regression matrices have significant unwanted
correlations.  Figures \ref{dreg3}, \ref{dreg4}, and \ref{dreg5} show
the LW, MW, and SW direct regression matrices.  The LW is marginal,
the MW looks bad, and the SW looks like garbage.  As noted in section
\ref{airs2cris} the 7377 profile dependent set is highly correlated;
the effective dimension is only 260, so our regression is actually
underdetermined.  The independent set residuals are small because the
7377 profile set spans the 49 profile test set, in the sense that
2400 of 7377 left-singular vectors reduces the reconstruction
residual for the 49 profile set to our 0.02~K threshold.

This is a common problem with regression.  One fix is to add noise.
Recall that we generate true {\airs} and true {\cris} by convolving
a common set of high-resolution radiances.  For true {\airs} we can
simply add noise at the {\airs} NEdN spec.  But if we want true
{\cris} radiances with noise for testing or regression it does not
work well to simply add noise at the {\cris} NEdN spec.  This does
reduce correlations but increases residuals for the independent set
significantly.  To model NEdN for the AIRS to CrIS translation we
synthesize noise at the {\airs} NEdN spec, add it to the signal, run
it through the translation, and measure it.  Similarly, for testing
or regression it might be better to generate true {\airs}, generate
noise at the {\airs} NEdN spec, add this to true {\airs}, translate
the individual noise spectra to {\cris}, and add these to true
{\cris}.  The problem appears to be getting a reference translation
for the noise, and we do not pursue that further here.
...
Figures \ref{dreg6} shows residuals and \ref{dreg7}, \ref{dreg8} and
\ref{dreg9} the final linear transform $R$ for the {\cris} LW, MW,
and SW bands.  We have chosen $i = j = 500$ for the LW, $i = 500$
and $j = 320$ for the MW, and $i = j = 100$ for the SW, to roughly
balance unwanted correlation with residual size.
...
$X$.  In practice this does not work as well as doing regression
