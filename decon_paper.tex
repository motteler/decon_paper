...
We can get such a one-step tranform in other ways.  For example if
$r_a$ and $r_c$ are $m \times k$ and $n \times k$ {\airs} and
{\cris} radiance sets, we can find $X$ to minimize $\|X r_a -
r_c\|_2$.  Typically $k > m$, giving an apparently overdetermined
system, and we solve $r_a^t X^t = r_c^t$ for $X$ by regression.
This is different from the corrections of section \ref{airs2cris}
and \ref{airsL1d}; there regression was used to find linear or
...
sets.  The residuals are roughly comparable to the residuals from
...
SW.  However the regression matrices show significant off-diagonal
correlations.  Figure \ref{dreg3} shows this for the LW; the MW and
SW bands are worse.  The dependent set residuals are very small,
much less than the residuals for the independent set shown above.
This and the off-diagonal correlations are signs of overfitting.  
As noted in section \ref{airs2cris} the 7377 profile dependent set
is highly correlated.  The effective dimension (as defined in the
appendix) is only 260, and the regression is actually
underdetermined.
...
% This is probably going to be true for any large set of radiances,
...
This paper, a companion talk, and all the translation and test code
discussed here are available online at github.\vspace{-2mm}
